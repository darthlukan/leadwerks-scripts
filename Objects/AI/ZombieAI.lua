...
Script.senseradius = 20     --float "Hearing Range"
    ...
    for k, entity in pairs(entities) do
		if entity.script.teamid ~= nil and entity.script.teamid ~= 0 and entity.script.teamid ~= self.teamid then
			if entity.script.health > 0 then
				...
				if self.entity.world:Pick(
                    self.entity:GetPosition() + Vec3(0, 1.6, 0),
                    entity:GetPosition() + Vec3(0, 1.6, 0),
                    pickinfo, 0, false, Collision.LineOfSight) == false then
...
function Script:DistanceToTarget()
    ...
    if math.abs(targetpos.y - pos.y) < 1.5 then
        ...
        --if they are on different vertical levels, assume they can't be reached
		return 100000
    end
end
...
function Script:Start()
    ...
    if self.alertsound then 
        self.sound.alert = Sound:Load(self.alertsound) 
    end

	self.sound.attack = {}

    if self.attack1sound then 
        self.sound.attack[1] = Sound:Load(self.attack1sound) 
    end
	
    if self.attack2sound then 
        self.sound.attack[2] = Sound:Load(self.attack2sound) 
    end
	
    if self.idlesound then 
        self.sound.idle = Sound:Load(self.idlesound) 
    end
...
function Script:EndAttack()
	if self.mode == "attack" then	
		...
		self.animationmanager:SetAnimationSequence("Attack"..tostring(1 + self.attackmode), 
            0.05, 300, 1, self, self.EndAttack)
